...
// Removes HTML tags like <p> or <b> from descriptions (Common in TVMaze)
...
/* ============================================================
   API 4: TVMaze (Series Specialist - No Key)
============================================================ */
...
/* ============================================================
   API 5: iTunes Store (Movie Specialist - No Key)
============================================================ */
async function fetchITunes(query) {
  try {
    ...
    // Hack: iTunes returns small 100x100 images. Rewrite URL for High Res.
    const bigPoster = d.artworkUrl100 ? d.artworkUrl100.replace('100x100bb', '600x600bb') : null;
    ...
    return {
      ...
      poster: bigPoster,
      rating: d.contentAdvisoryRating ? `🛡️ ${d.contentAdvisoryRating}` : null,
...
/* ============================================================
   API 6: Wikipedia (Final Text Fallback)
============================================================ */
async function fetchWiki(query) {
  try {
    const url = `https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(query)}`;
    const r = await axios.get(url, { timeout: 4000 });
    if (!r.data || !r.data.extract) return null;
    ...
    return {
      ...
      actors: 'Unknown',
      plot: r.data.extract,
      poster: r.data.thumbnail?.source || null,
      rating: null,
      trailer: null
    };
  } catch { return null; }
...
export default {
  ...
  command: {
    pattern: 'movie',
    desc: 'Search Movies/Series (6-Layer Fallback)',
    category: 'media',
    react: '🎬',
    ...
    run: async ({ sock, msg, args }) => {
      ...
      if (!query) {
        return sock.sendMessage(chat, { text: '🎬 *Usage:* .movie <Title>\nExample: .movie Deadpool' }, { quoted: msg });
      }
      ...
      if (!info) info = await fetchWiki(query);

      // === RESULTS ===
      if (!info) {
        return sock.sendMessage(chat, { text: `❌ No results found for *"${query}"* across 6 databases.` }, { quoted: msg });
      }
